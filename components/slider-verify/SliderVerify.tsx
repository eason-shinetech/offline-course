...
interface SliderVerifyState {
  isMouseEnter: boolean;
  diff: number;
}

export default class SliderVerify extends React.Component<
  SliderVerifyProps,
  SliderVerifyState
> {
  /**
  ...
  static defaultProps = {
    ...
    movedColor:
      "linear-gradient(313deg, rgba(65, 209, 102, 1) 0%, rgba(90, 232, 118, 1) 100%)",
    successTips: "完成验证",
  ...
  constructor(props: SliderVerifyProps) {
    super(props);
    this.state = {
      /** 是否滑入 */
      isMouseEnter: false,
  ...
  public componentDidMount() {
    ...
    this.max = (this.containerRef.current?.clientWidth || 0) - 50;
    this.disabled = false;
  }
  ...
  private mouseenter() {
    ...
    if (this.isSuccess || this.disabled) {
      return;
    }
    this.setState({
      isMouseEnter: true,
  ...
  private mouseleave() {
    this.disabled = this.props.disabled;
    if (this.isSuccess || this.isMousedown || this.disabled) {
      return;
    }
    this.setState({
      isMouseEnter: false,
  ...
  private mousemove(e: any) {
    this.disabled = this.props.disabled;
    if (!this.isMousedown || this.isSuccess || this.disabled) {
      return;
    }
    e.preventDefault();
    e.stopPropagation();
  ...
  public render() {
    ...
    const slideStyle = {
      ...
      left: 50 - (this.containerRef.current?.clientWidth || 0),
